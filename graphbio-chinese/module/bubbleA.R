  ...
  moduleServer(
    ...
    function(input, output, session) {
      ...
      dataModal <- function(failed = FALSE) {
        ...
        d[is.na(d)] <- ""
        modalDialog(
          span('1. 该图可以全面展示显著富集的GO term等信息,圆圈越大，代表越显著，最里面显著性越高，最外面显著性越低，显著性从圆心开始，呈现逆时针降序。'),
          tags$br(),
          span('2. 例子文件只显示部分，格式如下：第1列为GO term,第2列为-log10(FDR)并按值由大到小排序,第3列为图上需要显示的标签，例子仅标记了top13。表头信息不能变，只能为id、value、show!'),
          tags$br(),
          span('3. 注意：上传文件为逗号分隔文件（可用excel保存为csv格式，不是csv UTF-8!)'),
          tags$hr(),
