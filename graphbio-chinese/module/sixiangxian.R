...
sixiangxianUI <- function(id) {
  ...
  tagList(
    fluidRow(
        box(title="四象限图",solidHeader=TRUE,status='primary',background = "white",
        ...
        box(width=4,
          ...
          fileInput(ns("file1"),NULL,
                    ...
                    accept = c("text/csv",
                             ...
                             ".csv")),
          numericInput(ns("fc1"), label = "第1个倍数变化(Y轴)", value = 0),
          numericInput(ns("fc2"), label = "第2个倍数变化(X轴)", value = 0),
          numericInput(ns("ps"), label = "调整点大小", value = 1),
          sliderTextInput(
             inputId = ns("tm"),
             label = "调整透明度", 
             choices = seq(from = 0,
  ...
  moduleServer(
    ...
    function(input, output, session) {
      ...
      dataModal <- function(failed = FALSE) {
        ...
        modalDialog(
          span('第1列为基因ID，第2列为log2倍数，第3列为校正p值，第4列为另一组学数据倍数变化值，第5列为校正p值,第6列为需标记基因，若不需要标记，此列留空即可。'),
